...
public class ViewRole1Home {
	...
	// GUI Area 2: Student Discussion System - post list, search, action buttons
	protected static Label label_Search = new Label("Search:");
	protected static TextField text_SearchKeyword = new TextField();
	protected static ComboBox<String> combobox_ThreadFilter = new ComboBox<>();
	...
	protected static TableView<Post> tableView_Posts = new TableView<>();
	protected static Label label_EmptyMessage = new Label("No posts available. Create the first post!");
	protected static ObservableList<Post> postsList = FXCollections.observableArrayList();
	...
	public static void displayRole1Home(Stage ps, User user) {
		...
		theStage.setTitle("Student Discussion System");
	...
	private ViewRole1Home() {
		...
		text_SearchKeyword.setLayoutY(105);
		text_SearchKeyword.setPrefWidth(150);
		combobox_ThreadFilter.getItems().addAll("<All Threads>", "General");
		...
		button_Search.setOnAction((event) -> {ControllerRole1Home.performSearchPosts(); });
		setupButtonUI(button_CreatePost, "Dialog", 14, 110, Pos.CENTER, 20, 142);
		button_CreatePost.setOnAction((event) -> {ControllerRole1Home.performCreatePost(); });
		...
		button_Refresh.setOnAction((event) -> {ControllerRole1Home.performRefresh(); });
		...
		label_EmptyMessage.setLayoutX(300);
		label_EmptyMessage.setLayoutY(330);
		...
		// Place all of the widget items into the Root Pane's list of children
         theRootPane.getChildren().addAll(
			...
			button_CreatePost, button_ViewPost, button_Refresh, tableView_Posts,
