 ...
 * 
 * <p> Copyright: Lynn Robert Carter Â©️ 2025 </p>
 * 
...
public class ViewUserUpdate {
    ...
    private ViewUserUpdate() {
        ...
        dialogUpdatePassword = new TextInputDialog(""); // â ADDED
        ...
        dialogUpdatePassword.setTitle("Update Password"); // â ADDED
        dialogUpdatePassword.setHeaderText("Update your Password"); // â ADDED
        ...
        button_UpdatePassword.setOnAction((event) -> {
            ...
            result.ifPresent(newPw -> {
                ...
                String error = validateNewPassword(newPw, theUser.getPassword());
                if (error != null) {
                    // Simple JavaFX alert (you can swap with your projectâs existing alert style)
                    javafx.scene.control.Alert a =
                        new javafx.scene.control.Alert(javafx.scene.control.Alert.AlertType.ERROR);
                    a.setTitle("Invalid Password");
                    a.setHeaderText("Password update failed");
                    a.setContentText(error);
                    a.showAndWait();
                    return;
                }

                theDatabase.updatePassword(theUser.getUserName(), newPw);
                theDatabase.getUserAccountDetails(theUser.getUserName());

                String updated = theDatabase.getCurrentPassword();
                theUser.setPassword(updated);

                if (updated == null || updated.length() < 1) label_CurrentPassword.setText("<none>");
                else label_CurrentPassword.setText(updated); // (optional: mask it instead)
            });
        });
        ...
        theRootPane.getChildren().addAll(
                ...
                button_UpdateUsername,         // â ADDED (you created it but never added it)
